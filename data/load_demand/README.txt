...
run steps 7 and 8 to generate the load demand scenarios. Steps:
...
4.	Run popraw.py reads all the input files (.csv files) and populates the
    eb_data_raw table in the astrodek.sqlite database.
5.	Run popclean.py create unique building, season, day and daytime entries
6.	Run popload.py  populates the load of each unique building, season, day and 
    daytime entries
7.	Fill the results.csv file in the DATA folder to give the parameters to the
    simulation
8.	Run results_data.py to populates the results_data table in the
...
9.	Run results.py to generate the aggregate load scenario for the data and
    inputs given.
