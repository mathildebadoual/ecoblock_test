...
sql_script =    ('''SELECT time, demand, ev_demand, pv_generation  FROM results WHERE simulation_id = 1 and simulation_num = 1 ORDER BY id DESC''')
...
axes_array[1].plot(date_list, y_values_pv, label="PV")
axes_array[1].set_ylabel("kWh", fontsize=fs)
axes_array[1].tick_params(axis='both', which='major', labelsize=fs)
axes_array[1].set_ylim([0,250])
axes_array[1].set_yticks(np.arange(0,250,50))
axes_array[1].legend(fontsize=fs)

axes_array[2].plot(date_list, y_values_ev, label="EV")
axes_array[2].set_ylabel("kWh", fontsize=fs)
axes_array[2].tick_params(axis='both', which='major', labelsize=fs)
axes_array[2].set_ylim([0,50])
axes_array[2].set_yticks(np.arange(0,50,10))
axes_array[2].legend(fontsize=fs)

axes_array[2].xaxis.set_major_locator( MinuteLocator(np.arange(0,60,60)) )
axes_array[2].xaxis.set_major_formatter( DateFormatter('%H') )
axes_array[2].fmt_xdata = DateFormatter('%H')
