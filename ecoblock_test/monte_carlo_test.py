...
class System:
    ...
    def plot_results(self):
        ...
        to_plot = [(self.load_from_grid, 'load from grid'), (self.load_to_grid, 'load to grid'), (self.flywheel.soc_record, 'flywheel load'),
                (self.load_demand, 'load demand'), (self.ev_demand, 'ev demand'), (self.pv_generation, 'pv generation')]
        plt.figure(figsize=(10, 5))
        print(self.load_from_grid.shape)
        for i in range(len(to_plot)):
            plt.plot(dates, to_plot[i][0])
            #ax.xaxis.set_major_formatter(date_formater)
            plt.title(to_plot[i][1])
            plt.xlabel('time')
            plt.show()
